...
QuotaDock は Claude アカウントを最大 5 つまで並べて表示します。**設定 → 接続** の最後の Claude
カードで `+` を押すとアカウントが増え、`−` で最後のアカウントを外せます。アカウントごとに表示名と
色を持てます。Claude の各カードには接続方法のボタンが 3 つあります。アカウントごとに 1 つを選びます。

| 方法 | 読み取るもの | こんなとき |
|---|---|---|
| `CLI` | この PC の Claude Code がすでに保持している認証情報 | ここで Claude Code を使っている場合。CLI のログインは 1 つなので**アカウントは 1 つだけ**です。 |
| `Auth` | QuotaDock 自身のブラウザーウィンドウで行った Claude ログイン | 2 つ目のアカウント、または Claude Code のない PC。 |
| `その他` | 環境変数 `CLAUDE_CODE_OAUTH_TOKEN` | 自分で管理する長期トークン —— たとえば SSH で入る機器。 |

`その他` には貼り付け欄がありません —— 意図的にそうしています。QuotaDock はトークンの直接入力を
求めません。使い方：

1. ターミナルで `claude setup-token` を実行し、案内に従います。長期トークンが表示されます
   （Claude のサブスクリプションが必要です）。
2. その値を**ユーザー環境変数** `CLAUDE_CODE_OAUTH_TOKEN` として保存します
   （Windows: *設定 → システム → バージョン情報 → システムの詳細設定 → 環境変数*）。
3. QuotaDock を再起動します —— 環境変数はプログラム起動時に読み込まれます —— そのあと Claude
   カードの `その他` を押します。カードに「環境変数で構成済み」と表示されます。

この変数は実行するあらゆるプログラムから読めるため、パスワードと同じように扱ってください。共有する
スクリプトには入れず、漏れた場合は Anthropic アカウントで失効させてください。
