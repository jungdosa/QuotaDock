...
QuotaDock 最多可并排显示五个 Claude 账号。在 **设置 → 连接** 的最后一张 Claude 卡片上点 `+` 添加账号，
点 `−` 移除最后一个；每个账号都有自己的显示名和颜色。每张 Claude 卡片上有三个连接方式按钮，每个账号选一种。

| 方式 | 读取什么 | 适用场景 |
|---|---|---|
| `CLI` | 本机 Claude Code 已保存的凭据 | 你在这台电脑上使用 Claude Code。CLI 只保存一个登录，所以**只能一个账号**。 |
| `Auth` | 在 QuotaDock 自带浏览器窗口中完成的 Claude 登录 | 第二个账号，或没有 Claude Code 的电脑。 |
| `其他` | 环境变量 `CLAUDE_CODE_OAUTH_TOKEN` | 你自己管理的长期令牌 —— 例如通过 SSH 登录的机器。 |

`其他` 没有粘贴框 —— 这是有意的，QuotaDock 从不要求你输入令牌。使用步骤：

1. 在终端运行 `claude setup-token` 并按提示操作，会输出一个长期令牌（需要 Claude 订阅）。
2. 把它保存为名为 `CLAUDE_CODE_OAUTH_TOKEN` 的**用户环境变量**
   （Windows：*设置 → 系统 → 系统信息 → 高级系统设置 → 环境变量*）。
3. 重新启动 QuotaDock —— 环境变量在程序启动时读取 —— 然后点 Claude 卡片上的 `其他`。
   卡片会显示该变量已配置。

这个变量对你运行的任何程序都可见，请像对待密码一样对待它：不要写进共享的脚本，泄露后请到
Anthropic 账号中吊销。
