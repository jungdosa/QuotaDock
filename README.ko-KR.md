...
### Claude를 연결하는 세 가지 방법

QuotaDock은 Claude 계정을 최대 5개까지 나란히 보여줍니다. **설정 → 연결**의 마지막 Claude
카드에서 `+`로 계정을 추가하고 `−`로 마지막 계정을 뺍니다. 계정마다 표시명과 색을 따로 둘 수
있습니다. Claude 카드마다 연결 방식 버튼이 셋 있습니다. 계정마다 하나를 고릅니다.

| 방식 | 읽는 것 | 이럴 때 |
|---|---|---|
| `CLI` | 이 PC의 Claude Code가 이미 보관 중인 자격 증명 | 여기서 Claude Code를 쓰는 경우. CLI 로그인은 하나뿐이라 **계정 하나만** 가능합니다. |
| `Auth` | QuotaDock 자체 브라우저 창에서 한 Claude 로그인 | 두 번째 계정이거나, Claude Code가 없는 PC. |
| `기타` | 환경변수 `CLAUDE_CODE_OAUTH_TOKEN` | 직접 관리하는 장기 토큰 — 예를 들어 SSH로 들어가는 장비. |

`기타`에는 붙여넣을 칸이 없습니다 — 일부러 그렇습니다. QuotaDock은 토큰을 직접 입력하라고
요구하지 않습니다. 쓰는 순서:

1. 터미널에서 `claude setup-token`을 실행하고 안내를 따릅니다. 장기 토큰이 출력됩니다
   (Claude 구독이 필요합니다).
2. 그 값을 **사용자 환경변수** `CLAUDE_CODE_OAUTH_TOKEN`으로 저장합니다
   (Windows: *설정 → 시스템 → 정보 → 고급 시스템 설정 → 환경 변수*).
3. QuotaDock을 다시 시작합니다 — 환경변수는 프로그램이 시작될 때 읽힙니다 — 그다음 Claude
   카드의 `기타`를 누릅니다. 카드가 환경변수가 구성됐다고 표시합니다.

이 변수는 실행하는 모든 프로그램이 읽을 수 있으니 비밀번호처럼 다루십시오. 공유하는
스크립트에 넣지 말고, 유출됐다면 Anthropic 계정에서 폐기하십시오.
