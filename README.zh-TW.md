...
QuotaDock 最多可並排顯示五個 Claude 帳號。在 **設定 → 連線** 的最後一張 Claude 卡片按 `+` 新增帳號，
按 `−` 移除最後一個；每個帳號都有自己的顯示名稱與顏色。每張 Claude 卡片上有三個連線方式按鈕，每個帳號選一種。

| 方式 | 讀取什麼 | 適用情境 |
|---|---|---|
| `CLI` | 這台電腦的 Claude Code 已保存的憑證 | 你在這台電腦上使用 Claude Code。CLI 只保存一個登入，所以**只能一個帳號**。 |
| `Auth` | 在 QuotaDock 自帶的瀏覽器視窗完成的 Claude 登入 | 第二個帳號，或沒有 Claude Code 的電腦。 |
| `其他` | 環境變數 `CLAUDE_CODE_OAUTH_TOKEN` | 你自己管理的長期權杖 —— 例如透過 SSH 登入的機器。 |

`其他` 沒有貼上欄位 —— 這是刻意的，QuotaDock 從不要求你輸入權杖。使用步驟：

1. 在終端機執行 `claude setup-token` 並依提示操作，會輸出一個長期權杖（需要 Claude 訂閱）。
2. 把它存為名為 `CLAUDE_CODE_OAUTH_TOKEN` 的**使用者環境變數**
   （Windows：*設定 → 系統 → 關於 → 進階系統設定 → 環境變數*）。
3. 重新啟動 QuotaDock —— 環境變數在程式啟動時讀取 —— 然後按 Claude 卡片上的 `其他`。
   卡片會顯示該變數已設定。

這個變數對你執行的任何程式都可見，請像對待密碼一樣對待它：不要寫進共用的指令碼，外洩後請到
Anthropic 帳號撤銷。
