...
> **Linux는 영구 보류다 (2026-08-05 대표 결정).** 잔여작업에서 제외한다. 트레이 구현이
> 데스크톱 환경별로 갈려 대상 환경을 먼저 정해야 하는 부담이 컸고, 되살릴 계획이 없다.

- 서명·공증 절차는 `docs/reference/macos-signing-notes.md` 참고. Fyne 검증 게이트의
  macOS 항목(frameless 창, 트레이, 다중 모니터)을 먼저 통과시킨다.
- **Windows에서 크로스 빌드할 수 없다.** CGO가 Apple SDK와 clang을 요구하므로 빌드·테스트는
  macOS 장비에서 해야 한다. 코드 작성은 어디서든 가능하지만 검증은 그쪽 clone에서 한다.
- **별도 원격 clone**으로 작업하고, 클라우드 동기화 폴더 안에서 빌드하지 않는다.
- 선행 작업: `internal/platform/windows/native_other.go`의 스텁(프레임리스 창·항상 위·트레이·
  위치 복원·DPI·단일 인스턴스·자동 시작·둥근 모서리)이 전부 "지원 안 함"을 반환한다.
  macOS 구현체를 새로 써야 하며, 이것이 이 확장의 실제 분량이다.
